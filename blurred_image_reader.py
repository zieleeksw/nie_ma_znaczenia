...
def preprocess_image(image, method: str):
    ...
    if method == "median_blur":
        ...
        return cv2.threshold(filtered, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
    elif method == "adaptive_gaussian":
        blurred = cv2.GaussianBlur(image, (5, 5), 0)
        return cv2.adaptiveThreshold(blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 31, 2)
    elif method == "adaptive_bilateral":
        filtered = cv2.bilateralFilter(image, 9, 75, 75)
        return cv2.adaptiveThreshold(filtered, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 31, 2)
